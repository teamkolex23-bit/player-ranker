...
st.markdown(
    "Upload a single HTML file containing a players table (header row + data rows). "
    "Columns should use FM-style abbreviations (e.g. Name, Position, Age, Transfer Value, Pac, Acc, Dri, ...)."
)

# canonical attributes (no duplicate entries)
...
# Weights exactly as you supplied in the weighing table (roles and attributes)
WEIGHTS_BY_ROLE = {
    "GK": {
...
# abbreviation mapping that matches the HTML you gave and the canonical names
ABBR_MAP = {
...
def parse_players_from_html(html_text: str):
    ...
    # header cells — support both <th> and a first <tr> with <td>
    ...
    # coerce numeric-like columns (except textual ones)
    for c in df.columns:
    ...
    for col in unique_order:
        ...
        if len(same_cols) == 1:
            ...
            if subset_num.notna().sum().sum() > 0:
                ...
                # pick first non-empty string per row
...
if not uploaded:
    st.info("Upload an HTML file (export from FM as HTML). This app only accepts HTML tables.")
    st.stop()
...
# merge duplicate columns into one if necessary (fixes pyarrow / Streamlit duplicate-column error)
df = merge_duplicate_columns(df)

# compute ranking for chosen role
...
# find attributes present in both canonical list and parsed df
...
# normalization controls (in background ranking uses normalized values if checked)
...
attrs_df = df[available_attrs].fillna(0).astype(float, errors="ignore")
attrs_norm = attrs_df / float(max_val) if normalize else attrs_df

# create weight vector aligned with available_attrs
...
# Show Name, Position, Age, Transfer Value and Score, sorted by Score
cols_to_show = [c for c in ["Name", "Position", "Age", "Transfer Value", "Score"] if c in df_out_sorted.columns]
display_cols = cols_to_show + [c for c in available_attrs if c in df_out_sorted.columns]
st.subheader(f"Top players for role: {role} (sorted by Score)")
st.dataframe(df_out_sorted[display_cols].head(200))
